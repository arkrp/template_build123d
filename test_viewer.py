...
###
def makething(offset):
    with BuildPart() as part:
        with BuildSketch(Plane.XY) as sketch_1:
            with BuildLine() as line:
                Polyline((0,0), (1,0), (1,1), (0,1), close=True)
            make_face()
        my_extrude = extrude(amount=0.2)
        next_face = my_extrude.faces().sort_by(Axis.Z)[-1]
        with BuildSketch(next_face) as sketch_2:
            with BuildLine() as line:
                Polyline((0+offset,0+offset), (1+offset,0+offset), (1+offset,1+offset), (0+offset,1+offset), close=True)
            make_face()
        my_extrude = extrude(amount=0.5)
    return(part)
###
show(makething(0))
show(makething(0.4))
...
def makething2():
    with BuildPart() as part:
        with BuildSketch(Plane.XY) as sketch_1:
            a = project_vertex(Origin, sketch_1)
            b = project_vertex(Corner_1, sketch_1)
            c = project_vertex(Corner_2, sketch_1)
            with BuildLine() as line:
        ...
        next_face = my_extrude.faces().sort_by(Axis.Z)[-1]
        with BuildSketch(next_face) as sketch_2:
            a = project_vertex(Origin, sketch_2)
            b = project_vertex(Origin + (Corner_1 - Origin) * 0.9, sketch_2)
            c = project_vertex(Corner_2, sketch_2)
            with BuildLine() as line:
                Polyline(a, b, c, close=True)
            make_face()
        my_extrude = extrude(amount=0.2)
